...
class YoLov5TRT(object):
    ...
    def infer(self, raw_image_generator, vis=True):
        ...
        output = np.reshape(output, [self.batch_size, self.num_anchors, self.num_classes+5])
        ...
        for i in range(self.batch_size):
            ...
            if vis:  # 可视化
                ...
                for j in range(len(top_boxes)):
                    box = top_boxes[j]
                    plot_one_box(box,batch_image_raw[i],label="{}:{:.2f}".format(categories[int(top_label[j])], top_conf[j]),)
    ...
    def preprocess_image(self, raw_bgr_image):
        ...
        h, w, _ = image_raw.shape  # origin_size
        ...
        image = cv2.copyMakeBorder(
            image, top, bottom, left, right, cv2.BORDER_CONSTANT, None, (128, 128, 128)
        )
